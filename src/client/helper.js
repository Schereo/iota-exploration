...
export const client = new ClientBuilder().node(process.env.MQTT_NODE_UR).build();
...
export const getAddress = async (seed) => {
  const address = await client.getAddresses(seed).accountIndex(0).range(0, 1).get();
  return address[0];
};

export const getAddressBalance = async (addr) => {
  const { address, balance, dustAllowed } = await client.getAddressBalance(addr);
  return { address, balance, dustAllowed };
};

export const getSeedBalance = async (seed) => {
  const balance = await client.getBalance(seed).accountIndex(0).initialAddressIndex(0).get();
  return balance;
};
...
export const sendTokens = async (senderSeed, receiverAddress, amount) => {
  const message = await client.message().seed(senderSeed).output(receiverAddress, amount).submit();
  return message;
};

export const sendDust = async (senderSeed, receiverAddress, amount) => {
  const message = await client.message().seed(senderSeed).dustAllowanceOutput(receiverAddress, amount).submit();
  return message;
};

export const findOutputs = async (addr) => {
  const outputs = await client.findOutputs([], [addr]);
  return outputs;
};
